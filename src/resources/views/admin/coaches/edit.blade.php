...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.coach.title_singular') }}
    </div>
        ...
        <form action="{{ route("admin.coaches.update", [$coach->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('coach_gender') ? 'has-error' : '' }}">
                ...
                @foreach(App\Coach::COACH_GENDER_RADIO as $key => $label)
                    <div>
                        <input id="coach_gender_{{ $key }}" name="coach_gender" type="radio" value="{{ $key }}" {{ old('coach_gender', $coach->coach_gender) === (string)$key ? 'checked' : '' }} required>
            ...
            <div class="form-group {{ $errors->has('coach_password') ? 'has-error' : '' }}">
                ...
                <input type="password" id="coach_password" name="coach_password" class="form-control">
