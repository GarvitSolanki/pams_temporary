...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.spoc.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("admin.spocs.update", [$spoc->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('spoc_gender') ? 'has-error' : '' }}">
                ...
                @foreach(App\Spoc::SPOC_GENDER_RADIO as $key => $label)
                    <div>
                        <input id="spoc_gender_{{ $key }}" name="spoc_gender" type="radio" value="{{ $key }}" {{ old('spoc_gender', $spoc->spoc_gender) === (string)$key ? 'checked' : '' }} required>
            ...
            <div class="form-group {{ $errors->has('spoc_password') ? 'has-error' : '' }}">
                ...
                <input type="password" id="spoc_password" name="spoc_password" class="form-control">
