...
@can('project_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            ...
            <button class="btn btn-warning" data-toggle="modal" data-target="#assignProjectModal">
                {{ trans('global.app_assignProject') }}
            </button>
            @include('csvImport.modal', ['model' => 'Project', 'route' => 'admin.projects.parseCsvImport'])
            @include('admin.projects.assign', ['college' => 'College', 'route' => 'admin.assign.index'])
        </div>
    </div>
@endcan
<div class="card">
                        ...
                        <tr data-entry-id="{{ $project->id }}">
                            ...
                            <td>
                                @if($project->project_media)
                                    @foreach($project->project_media as $key => $media)
                                            ...
                                            <a class="align-items-baseline nav-item" href="{{ substr($media->getUrl() , 16 ) }}" target="_blank">
                                                <span class="badge badge-dark">{{ $loop->iteration }}</span>
                                                <i class="m-2 text-justify text-dark text-capitalize"> {{ $media->name }} </i>
                                                <hr>
                                            </a>
                            ...
                            <td>
                                @if($project->project_dataset)
                                    @foreach($project->project_dataset as $key => $media)
                                        <a class=" nav-fill" href="{{ substr($media->getUrl() , 16 ) }}" target="_blank">
                                            {{ $media->name }}
                                        </a>
                                    @endforeach
                                @endif
